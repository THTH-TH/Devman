    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-5 right-5 z-40 w-12 h-12 bg-violet-600 hover:bg-violet-700 text-white rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-105 active:scale-95"
          title="AI Assistant"
        >
          <Sparkles size={20} />
        </button>
      ...
      {open && (
        <div className="fixed bottom-5 right-5 z-50 w-[380px] max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden" style={{ height: '520px' }}>
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-violet-600 text-white shrink-0">
                ...
                <div className="space-y-2">
                  {SUGGESTIONS.map(s => (
                    <button
                      ...
                      className="w-full text-left text-xs text-gray-600 bg-gray-50 hover:bg-violet-50 hover:text-violet-700 px-3 py-2 rounded-lg border border-gray-100 hover:border-violet-200 transition-colors"
                    >
                      {s}
                    </button>
                  ))}
          ...
          <div className="px-3 py-3 border-t border-gray-100 shrink-0">
            <div className="flex items-end gap-2 bg-gray-50 rounded-xl border border-gray-200 px-3 py-2 focus-within:border-violet-400 focus-within:ring-1 focus-within:ring-violet-400 transition-all">
              <textarea
                ref={inputRef}
                rows={1}
                value={input}
              ...
              <button
                ...
                className="w-7 h-7 bg-violet-600 hover:bg-violet-700 disabled:opacity-40 text-white rounded-lg flex items-center justify-center transition-colors shrink-0"
              >
                <Send size={13} />
              </button>
            </div>
