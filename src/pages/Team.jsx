...
const AVATAR_COLORS = [
  'bg-forest-500', 'bg-teal-500', 'bg-forest-600', 'bg-pink-500',
    ...
    return (
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="text-center py-20 text-gray-400 text-sm">
          No team members yet.{' '}
          <Link to="/settings" className="text-forest-600 hover:underline">Add your team in Settings.</Link>
        </div>
      </div>
    )
  }
        ...
        {enriched.map(member => (
          <div key={member.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-shadow">
            ...
            <div className="space-y-1.5 mb-4">
              {member.email && (
                <a href={`mailto:${member.email}`} className="flex items-center gap-2 text-xs text-gray-500 hover:text-forest-600 transition-colors">
                  <Mail size={12} className="shrink-0 text-gray-400" />
                  <span className="truncate">{member.email}</span>
                </a>
              )}
              {member.phone && (
                <a href={`tel:${member.phone}`} className="flex items-center gap-2 text-xs text-gray-500 hover:text-forest-600 transition-colors">
            ...
            {member.openTasks > 0 && (
              <Link
                ...
                className="mt-3 flex items-center gap-1.5 text-xs text-forest-600 hover:underline"
              >
                <ListTodo size={12} />
                View their tasks
              </Link>
